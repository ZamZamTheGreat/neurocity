...
import { auditEvents, checkoutGroups, orders, orderStatusEvents, serviceBookings } from "../db/schema";
...
export async function expireDueOrderRequests(limit = 40, at = new Date()) {
  ...
  for (const candidate of groups) {
    await db.transaction(async (tx) => {
      ...
      if (target === "payment_expired" && affected.length) await tx.update(serviceBookings).set({ status: "cancelled", paymentStatus: "expired", updatedAt: at }).where(inArray(serviceBookings.orderId, affected.map((order) => order.id)));
