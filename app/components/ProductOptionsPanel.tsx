...
import { productTemplateForCategory, usesColourwayTerminology } from "../../lib/product-category-templates";
...
export default function ProductOptionsPanel({
  ...
  const usesColourways = usesColourwayTerminology(product.category);
  const groupLabel = usesColourways ? "colourway" : "option group";
    ...
    <section className="product-options">
      <header>
        <div>
          ...
          <h3>{usesColourways ? "Colourways" : "Product options"}, choices and stock</h3>
          <p>
            {active
              ? `${active} active option${active === 1 ? "" : "s"}. Prices follow the product unless an ${groupLabel} has its own price.`
...
function ColourwayRow({ variants, onChange, onSave, onUpload, groupLabel, choiceLabel }: { variants: Variant[]; onChange: (variant: Variant) => void; onSave: (variant: Variant) => Promise<void>; onUpload: (file?: File) => Promise<boolean>; groupLabel: string; choiceLabel: string }) {
  ...
  return <article className="colourway-editor">
    <header>
      <div className="variant-image">{image ? <ManagedImage src={image} alt={`${color} ${groupLabel}`} /> : <span>No image</span>}<label>{image ? "Replace" : "Add image"}<input aria-label={`${image ? "Replace" : "Add"} ${color} ${groupLabel} image`} type="file" accept="image/jpeg,image/png,image/webp" onChange={(event) => { const file = event.target.files?.[0]; if (file) setCrop(file); event.currentTarget.value = ""; }} /></label></div>
      <div><small>{groupLabel.toUpperCase()}</small><h4>{color}</h4><span>{variants.length} choice{variants.length === 1 ? "" : "s"} · {choiceLabel}</span></div>
    ...
    <details className="colourway-advanced"><summary>Advanced settings by choice</summary><div>{variants.map((variant) => <VariantRow key={variant.id} variant={variant} onChange={onChange} onSave={() => onSave(variant)} onUpload={onUpload} choiceLabel={choiceLabel} optionLabel={groupLabel} />)}</div></details>
    <footer><button onClick={() => void Promise.all(variants.map(onSave))}>Save {groupLabel}</button></footer>
...
function NewVariantForm({
  ...
  const template = productTemplateForCategory(product.category);
  const sizeOptions = template.choices;
  const groupLabel = usesColourwayTerminology(product.category) ? "colourway" : "option group";
    ...
    <div className="new-variant-form">
      <div>
        <label>
          ...
          <input
            value={values.color}
            placeholder={template.optionPlaceholder}
            onChange={(event) => update({ color: event.target.value })}
          />
        </label>
        <fieldset className="size-multiselect">
          <legend>{template.choiceLabel}</legend>
          <details>
            <summary>{values.sizes.length ? `${values.sizes.length} choice${values.sizes.length === 1 ? "" : "s"} selected` : "Choose options"}</summary>
            <div>
              ...
              <div className="custom-size"><input value={customSize} placeholder="Custom value" onChange={(event) => setCustomSize(event.target.value)} /><button type="button" onClick={() => { const size = customSize.trim(); if (size && !values.sizes.includes(size)) update({ sizes: [...values.sizes, size] }); setCustomSize(""); }}>Add</button></div>
            </div>
          </details>
          <small>{template.choiceHelp} You can add a custom value.</small>
        </fieldset>
        <label>
          Regular price (N$)
          <input
      ...
      <p className="generated-sku-note">Prices are prefilled from the product and applied to every selected choice. Change them here only when this {groupLabel} has a different price. NeuroCity will create one inventory option and a unique SKU for every selected choice.</p>
      <footer>
        ...
        <button
          ...
          onClick={submit}
        >
          Create {values.sizes.length || ""} choice{values.sizes.length === 1 ? "" : "s"}
        </button>
      </footer>
    </div>
  );
