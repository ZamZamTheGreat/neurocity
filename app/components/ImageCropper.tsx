...
export default function ImageCropper({ file, aspect, width, title, onCancel, onApply }: { file: File; aspect: number; width: number; title: string; onCancel: () => void; onApply: (file: File) => void }) {
  ...
  return <div className="image-editor-backdrop" role="dialog" aria-modal="true" aria-label={title}>
    <section className="image-editor-panel">
      <header><div><small>IMAGE EDITOR</small><h3>{title}</h3></div><button onClick={onCancel} aria-label="Close image editor">×</button></header>
      ...
      <p>The highlighted frame is exactly how the image will appear.</p>
