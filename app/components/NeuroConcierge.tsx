...
export function NeuroConcierge({ open, onClose }: { open: boolean; onClose: () => void }) {
  ...
  useEffect(() => {
    if (!open || profile || signedOut) return;
    let cancelled = false;
  ...
  async function ask(message: string) {
    ...
    try {
      ...
      const result = await response.json();
      if (response.status === 401) { setSignedOut(true); setProfile(null); return; }
  ...
  return <div className="assistant-backdrop" onClick={(event) => { if (event.target === event.currentTarget) onClose(); }} role="presentation"><section className="assistant neuro-assistant" aria-label="Personal shopping companion"><header><div><img src="/branding/neurocity-mark.png" alt="" /><div><b>{profile?.companionName ?? "James"}</b><small><i /> Your NeuroCity shopping companion</small></div></div><div className="companion-header-actions">{profile && <button onClick={() => setRenaming(!renaming)} aria-label="Rename your companion">Rename</button>}<button onClick={onClose} aria-label="Close companion">×</button></div></header>{renaming && <form className="companion-rename" onSubmit={rename}><label>Name your companion<input value={newName} minLength={2} maxLength={40} onChange={(event) => setNewName(event.target.value)} /></label><button>Save name</button>{profileError && <small>{profileError}</small>}</form>}{signedOut ? <div className="companion-gate"><img src="/branding/neurocity-mark.png" alt="" /><p className="eyebrow">Personal to you</p><h2>Meet James, your shopping companion.</h2><p>Sign in to name your companion and receive catalogue recommendations connected to your NeuroCity account.</p><a href="/login?return_to=%2F">Sign in or create an account</a><small>Your future memories and preferences will remain tied to your account and under your control.</small></div> : profileLoading ? <div className="companion-loading"><span /><p>Opening your companion…</p></div> : profileError && !profile ? <div className="companion-gate"><h2>James is unavailable</h2><p>{profileError}</p><button onClick={() => { setProfileError(""); setSignedOut(false); }}>Try again</button></div> : <><div className="assistant-body neuro-thread" aria-live="polite">{messages.map((message) => <article className={`neuro-message ${message.role}`} key={message.id}><div>{message.role === "companion" && <span>{profile?.companionName.slice(0, 1).toUpperCase()}</span>}<p>{message.text}</p></div>{message.matches?.length ? <div className="neuro-results">{message.matches.map((match) => <article key={match.id}><img src={match.imageUrl ?? "/branding/neurocity-mark.png"} alt="" /><div><small>{match.store.name}{match.collection ? ` · ${match.collection}` : ""}</small><b>{match.name}</b><p>{match.colours.length ? match.colours.join(" · ") : "Published product"}{match.sizes.length ? ` · ${match.sizes.join("/")}` : ""}</p><strong>{money(match.price)}</strong>{match.availableUnits > 0 && <em>{match.availableUnits} available</em>}</div><a href={`/stores/${match.store.slug}#shop`}>View</a></article>)}</div> : null}</article>)}{busy && <div className="neuro-thinking"><span /><span /><span /><small>{profile?.companionName} is searching live stores</small></div>}<div ref={endRef} /></div>{messages.length === 1 && <div className="suggestions neuro-prompts">{prompts.map((prompt) => <button key={prompt} onClick={() => void ask(prompt)}>{prompt}</button>)}</div>}<form onSubmit={submit}><label><span className="sr-only">Describe what you need</span><input value={input} maxLength={300} onChange={(event) => setInput(event.target.value)} placeholder={`Ask ${profile?.companionName ?? "James"} what you need`} /></label><button disabled={busy || input.trim().length < 2}>{busy ? "…" : "Send"}</button></form><footer>{profile?.companionName} only recommends products currently published by approved stores.</footer></>}</section></div>;
}
