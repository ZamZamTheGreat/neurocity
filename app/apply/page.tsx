...
const fields = { legalName: "", tradingName: "", registrationNumber: "", businessType: "", category: "", description: "", representativeName: "", representativeRole: "", email: "", phone: "", password: "", confirmPassword: "", physicalAddress: "", website: "", socialProfiles: "", branchCount: 1, branchLocations: "", productSummary: "", estimatedProductCount: 1, pickupAvailable: false, deliveryAvailable: false, deliveryDetails: "", returnsPolicy: "", termsAccepted: false, privacyAccepted: false };
...
export default function ApplyPage() {
  ...
  const selectedCategory = merchantCategories.find((category) => category.name === form.category);
  return <main className="application-page"><header><a href="/" className="brand"><span>Neuro</span><strong>City</strong></a><a href="/">Return to mall</a></header><section className="application-intro"><p className="eyebrow">Sell with NeuroCity</p><h1>Apply for a merchant storefront</h1><p>Submit your business information and required documents in one secure application.</p><div><span>1 · Business</span><span>2 · Operations</span><span>3 · Documents</span><span>4 · Review</span></div></section>{reference ? <ApplicationDocuments reference={reference} /> : <form className="application-form" onSubmit={submit}><h2>Business identity</h2><Field label="Registered business name" value={form.legalName} onChange={(v) => update("legalName", v)} /><Field label="Trading name" value={form.tradingName} onChange={(v) => update("tradingName", v)} /><Field label="Registration number" value={form.registrationNumber} onChange={(v) => update("registrationNumber", v)} /><Field label="Business type" value={form.businessType} onChange={(v) => update("businessType", v)} placeholder="CC, company, sole proprietor…" /><label className="category-field">Main category<select required value={form.category} onChange={(event) => update("category", event.target.value)}><option value="" disabled>Select the best match</option>{merchantCategories.map((category) => <option key={category.name} value={category.name}>{category.icon} {category.name}</option>)}</select>{selectedCategory && <small>{selectedCategory.includes}</small>}</label><Field label="Business description" value={form.description} onChange={(v) => update("description", v)} wide />
  <h2>Authorised representative</h2><Field label="Full name" value={form.representativeName} onChange={(v) => update("representativeName", v)} /><Field label="Role" value={form.representativeRole} onChange={(v) => update("representativeRole", v)} /><Field label="Email" type="email" value={form.email} onChange={(v) => update("email", v)} /><Field label="Phone" value={form.phone} onChange={(v) => update("phone", v)} /><h2>Account security</h2><label>Password<input required type="password" minLength={10} autoComplete="new-password" value={form.password} onChange={(event) => update("password", event.target.value)} /><small>At least 10 characters. Use this email and password to return to your application.</small></label><label>Confirm password<input required type="password" minLength={10} autoComplete="new-password" value={form.confirmPassword} onChange={(event) => update("confirmPassword", event.target.value)} /></label><Field label="Physical business address" value={form.physicalAddress} onChange={(v) => update("physicalAddress", v)} wide /><Field label="Website" value={form.website} onChange={(v) => update("website", v)} /><Field label="Social profiles" value={form.socialProfiles} onChange={(v) => update("socialProfiles", v)} />
...
function ApplicationDocuments({ reference }: { reference: string }) {
  ...
  return <section className="document-upload-page integrated-documents"><p className="eyebrow">Application created</p><h2>Upload your required documents</h2><p className="application-reference">Reference <strong>{reference}</strong></p><p>Your account is active and you are signed in. Upload all four documents to complete the application.</p>{authRequired && <div className="upload-auth-notice"><strong>Your session has ended</strong><span>Sign in again using the application email and password you created.</span><a href={`/login?return_to=${encodeURIComponent(returnTo)}`}>Sign in to continue</a></div>}{requiredDocuments.map((item) => <article key={item.type}><div><h3>{item.label}</h3><p>{states[item.type] ?? "Required before final approval"}</p></div><label className="file-button">Choose file<input type="file" accept="application/pdf,image/jpeg,image/png" onChange={(event) => upload(item.type, event.target.files?.[0])} /></label></article>)}<div className="upload-progress"><strong>{completed} of 4 documents uploaded</strong>{completed === 4 ? <><span>Your application is complete and ready for review.</span><a href="/application-status">Track application status →</a></> : <span>You can safely return later using this reference.</span>}</div></section>;
}
