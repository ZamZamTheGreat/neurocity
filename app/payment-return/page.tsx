...
export default function PaymentReturnPage() {
  ...
  return <main className="payment-return-page"><section className={`payment-return-card ${message.tone}`}><span aria-hidden="true">{message.tone === "success" ? "✓" : message.tone === "failed" ? "!" : "…"}</span><small>PAYTODAY CHECKOUT</small><h1>{message.title}</h1><p>{message.copy}</p>{reference && <strong>{reference}</strong>}<div><Link href="/account?tab=Orders">Return to NeuroCity orders</Link><Link href="/" className="secondary">Go to home</Link></div><small>You can safely close the PayToday payment tab after returning to NeuroCity.</small></section></main>;
}
