...
export async function POST(request: Request) {
  ...
  try {
    ...
    if (payload.paymentMethod === "pay_on_collection" && paymentSettings.payOnCollectionEnabled === false) return Response.json({ error: "Pay on collection is not offered by this store." }, { status: 409 });
    if (payload.paymentMethod === "eft" && paymentSettings.eftEnabled !== true) return Response.json({ error: "EFT is not currently offered by this store." }, { status: 409 });
    if (payload.paymentMethod === "eft" && ![paymentSettings.bankName, paymentSettings.accountHolder, paymentSettings.accountType, paymentSettings.accountNumber, paymentSettings.branchCode].every((value) => typeof value === "string" && value.trim())) return Response.json({ error: "This store has not completed its EFT instructions." }, { status: 409 });
