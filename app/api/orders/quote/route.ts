...
export async function GET(request: Request) {
  ...
  const url = new URL(request.url), requestedMerchantId = url.searchParams.has("merchantId") ? Number(url.searchParams.get("merchantId")) : null, addressId = Number(url.searchParams.get("addressId"));
  if (requestedMerchantId !== null && (!Number.isInteger(requestedMerchantId) || requestedMerchantId <= 0)) return Response.json({ error: "Valid merchant required." }, { status: 400 });
  if (!Number.isInteger(addressId) || addressId <= 0) return Response.json({ error: "Choose a delivery address." }, { status: 400 });
  const db = getDb();
  ...
  const cartMerchants = requestedMerchantId !== null ? [requestedMerchantId] : [...new Set((await db.select({ merchantId: products.merchantId }).from(customerCartItems).innerJoin(productVariants, eq(productVariants.id, customerCartItems.variantId)).innerJoin(products, eq(products.id, productVariants.productId)).where(eq(customerCartItems.userId, Number(user.userId)))).map((item) => item.merchantId))];
  ...
  return Response.json({ supported: true, deliveryFee: quotes.reduce((sum, item) => sum + Number(item.zone?.fee ?? 0), 0), area: quotes[0].zone!.area, estimatedTime: quotes.map((item) => item.zone?.estimatedTime).filter(Boolean).join(" · "), stores: quotes.map((item) => ({ merchantId: item.merchant.id, name: item.merchant.name, fee: item.zone!.fee, estimatedTime: item.zone!.estimatedTime })) });
}
