...
export async function GET(_request: Request, context: { params: Promise<{ slug: string }> }) {
  ...
  return Response.json({ store: publicStore, branches: branches.map((branch) => ({ ...branch, hours: hours.filter((item) => item.branchId === branch.id) })), promotions, products: catalogue.map((product) => ({ ...product, imageUrl: product.imageUrl?.startsWith("r2://") ? `/api/stores/${encodeURIComponent(store.slug)}/media?type=product&productId=${product.id}` : product.imageUrl, variants: variants.filter((variant) => variant.productId === product.id).map((variant) => ({ ...variant, stock: stock.filter((item) => item.variantId === variant.id), available: stock.filter((item) => item.variantId === variant.id).reduce((total, item) => total + Math.max(0, item.onHand - item.reserved - item.safetyStock), 0) })) })) }, { headers: { "cache-control": "no-store" } });
}
