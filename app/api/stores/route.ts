...
export async function GET(request: Request) {
  ...
  return { platform, stores: ready.map((store) => { const branch = branches.find((item) => item.merchantId === store.id); return { ...store, status: "open", branchName: branch?.name ?? null, branchAddress: branch?.address ?? null, logoUrl: store.logoUrl?.startsWith("r2://") ? `/api/stores/${encodeURIComponent(store.slug)}/media?type=logo` : store.logoUrl, bannerUrl: store.bannerUrl?.startsWith("r2://") ? `/api/stores/${encodeURIComponent(store.slug)}/media?type=banner` : store.bannerUrl }; }) };
  });
  return Response.json(payload, { headers: publicCacheHeaders(30, 120) });
}
