...
export async function GET() {
  try {
    const payload = await cachedPublicData("malls:directory", 60, async () => { const db = getDb();
    ...
    return { malls: malls.map((mall) => { const members = assignments.filter((assignment) => assignment.tenantId === mall.id); const visible = members.filter((member) => { const merchant = merchantList.find((item) => item.id === member.merchantId); return merchant?.isPublic && ["active", "pilot"].includes(merchant.status); }); return { id: mall.id, name: mall.name, slug: mall.slug, city: mall.city, country: mall.country, tagline: mall.tagline, logoUrl: mall.logoUrl, markUrl: mall.markUrl, theme: mall.theme, features: mall.features, storeCount: visible.length, domain: domains.find((domain) => domain.tenantId === mall.id && domain.isPrimary)?.hostname ?? null }; }) };
