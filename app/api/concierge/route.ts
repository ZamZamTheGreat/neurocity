...
export async function POST(request: Request) {
  try {
    ...
    const ranked = catalogue.map((product) => {
      ...
      const availability = product.itemType === "service" ? "bookable" : availableUnits > 0 ? "in_stock" : product.availability === "preorder" ? "preorder" : "out_of_stock";
