...
export async function POST(request: Request) {
  try {
    ...
    const reply = selected.length ? `I found ${selected.length} live ${selected.length === 1 ? "match" : "matches"}${constraints ? ` for ${constraints}` : ""} on ${platform.name}. Prices, options and available units were checked just now.` : `I couldn't find an in-stock product on ${platform.name} matching every detail. Try widening the budget or removing one preference.`;
    ...
    return Response.json({ error: "James couldn't search the live catalogue right now. Please try again." }, { status: 500 });
  }
}
