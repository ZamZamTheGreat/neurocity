...
export async function POST(request: Request) {
  try {
    const { name, email, password, privacyAccepted, termsAccepted } = await request.json() as { name?: string; email?: string; password?: string; privacyAccepted?: boolean; termsAccepted?: boolean };
    ...
    if (privacyAccepted !== true && normalized !== adminEmail) return Response.json({ error: "Accept the NeuroCity privacy notice to create an account." }, { status: 400 });
    if (termsAccepted !== true && normalized !== adminEmail) return Response.json({ error: "Accept the NeuroCity Terms & Conditions to create an account." }, { status: 400 });
    const acceptedAt = new Date();
    const [user] = await db.insert(users).values({ email: normalized, displayName: name.trim(), passwordHash: await hash(password, 12), platformRole: normalized === adminEmail ? "administrator" : "customer", privacyNoticeVersion: PRIVACY_NOTICE_VERSION, privacyAcceptedAt: acceptedAt, termsVersion: TERMS_VERSION, termsAcceptedAt: acceptedAt }).returning();
