...
export async function POST(request: Request) {
  try {
    const { name, email, password, privacyAccepted } = await request.json() as { name?: string; email?: string; password?: string; privacyAccepted?: boolean };
    ...
    const [user] = await db.insert(users).values({ email: normalized, displayName: name.trim(), passwordHash: await hash(password, 12), platformRole: normalized === adminEmail ? "administrator" : "customer", privacyNoticeVersion: PRIVACY_NOTICE_VERSION, privacyAcceptedAt: new Date() }).returning();
