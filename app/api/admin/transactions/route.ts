...
export async function GET() {
  ...
  const gatewayTransactions = gatewayRows.map((row) => {
    ...
    const finalizedAt = typeof metadata.finalizedAt === "string" && !Number.isNaN(Date.parse(metadata.finalizedAt)) ? metadata.finalizedAt : null;
    return { ...row, capturedAt: row.status === "paid" ? finalizedAt ?? row.updatedAt : null, recordId: `gateway-${row.id}`, source: "gateway", paymentMethod: row.provider, customerName: relatedOrders[0]?.customerName ?? null, customerEmail: relatedOrders[0]?.customerEmail ?? null, orderReferences: relatedOrders.map((order) => `NC-${String(order.id).padStart(6, "0")}`), allocations: allocations.filter((allocation) => allocation.checkoutGroupId === row.checkoutGroupId).map(({ paymentSettings, ...allocation }) => ({ ...allocation, settlementAccount: settlementAccount(paymentSettings) })) };
  });
  const manualTransactions = orderRows.filter((order) => !order.checkoutGroupId || !gatewayCheckoutIds.has(order.checkoutGroupId)).map(({ paymentSettings, ...order }) => ({ id: null, checkoutGroupId: order.checkoutGroupId, checkoutReference: order.checkoutGroupId ? `Checkout ${order.checkoutGroupId}` : null, provider: order.paymentMethod ?? "manual", providerReference: null, amount: order.total, currency: "NAD", status: order.paymentStatus, failureMessage: null, lastCheckedAt: null, capturedAt: order.paymentStatus === "paid" ? order.updatedAt : null, createdAt: order.createdAt, updatedAt: order.updatedAt, recordId: `order-${order.id}`, source: "order", paymentMethod: order.paymentMethod ?? "manual", customerName: order.customerName, customerEmail: order.customerEmail, orderReferences: [`NC-${String(order.id).padStart(6, "0")}`], allocations: [{ id: 0, checkoutGroupId: order.checkoutGroupId, orderId: order.id, merchantId: order.merchantId, merchantName: order.merchantName, grossAmount: order.total, deliveryAmount: 0, platformFee: 0, providerFee: 0, netAmount: order.total, settlementStatus: "not_applicable", settlementDueAt: null, settlementReference: null, settledAt: null, settlementAccount: settlementAccount(paymentSettings) }] }));
...
function settlementAccount(value: unknown) {
  const settings = value && typeof value === "object" ? value as Record<string, unknown> : {};
  return {
    bankName: String(settings.bankName ?? ""),
    accountHolder: String(settings.accountHolder ?? ""),
    accountType: String(settings.accountType ?? ""),
    accountNumber: String(settings.accountNumber ?? ""),
    branchCode: String(settings.branchCode ?? ""),
    complete: [settings.bankName, settings.accountHolder, settings.accountType, settings.accountNumber, settings.branchCode].every(Boolean),
  };
}
