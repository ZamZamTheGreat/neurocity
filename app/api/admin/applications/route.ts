...
export async function PATCH(request: Request) {
  ...
  await db.transaction(async (tx) => {
    if (status === "approved" && !merchantId) { const [merchant] = await tx.insert(merchants).values({ name: application.tradingName, slug: slugify(application.tradingName, application.id), category: application.category, primaryCategory: application.category, enabledCategories: [application.category], offeringType: application.offeringType, locationType: application.locationType, mainOperatingArea: application.mainOperatingArea, status: "onboarding", contactName: null, contactEmail: null, contactPhone: null, website: null, pickupLocation: null, tagline: null, description: null, logoUrl: null, bannerUrl: null, policies: {}, contactOptions: {}, fulfillmentMethods: [], isPublic: false, setupStep: 1 }).returning(); merchantId = merchant.id; }
  ...
  const instructions = status === "approved" ? `Your application is approved. Sign in at ${appUrl}/login using ${application.email}, then open your merchant dashboard to complete setup.` : status === "more_information_required" ? `More information is required. Review note: ${notes || "Please contact NeuroCity."}` : status === "rejected" ? `The application was not approved. Review note: ${notes || "Contact NeuroCity for more information."}` : "Your application is now under review.";
