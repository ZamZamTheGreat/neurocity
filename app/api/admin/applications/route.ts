...
export async function GET() { const user = await getChatGPTUser(); if (user?.platformRole !== "administrator") return Response.json({ error: "Administrator access required." }, { status: 403 }); const db = getDb(); const applications = await db.select().from(merchantApplications).orderBy(desc(merchantApplications.submittedAt)); const documents = applications.length ? await db.select().from(applicationDocuments).where(inArray(applicationDocuments.applicationId, applications.map((item) => item.id))) : []; let storage: ReturnType<typeof getR2> | null = null; try { storage = getR2(); } catch { /* storage links remain unavailable until configured */ } const withDocuments = await Promise.all(applications.map(async (application) => ({ ...application, documents: await Promise.all(documents.filter((document) => document.applicationId === application.id).map(async (document) => ({ ...document, viewUrl: storage && document.status === "uploaded" && document.storageKey ? await getSignedUrl(storage.client, new GetObjectCommand({ Bucket: storage.bucket, Key: document.storageKey, ResponseContentDisposition: `inline; filename="${document.originalName ?? "document"}"` }), { expiresIn: 300 }) : null }))) }))); return Response.json({ applications: withDocuments }); }
