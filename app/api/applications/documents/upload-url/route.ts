...
import { allowedDocumentMimeTypes, documentTypes, getR2, maxDocumentBytes } from "../../../../../lib/r2";
...
export async function POST(request: Request) {
  ...
  const safeName = filename.replace(/[^a-zA-Z0-9._-]+/g, "-").slice(-120); const key = `applications/${application.id}/${documentType}/${randomUUID()}-${safeName}`; const { client, bucket } = getR2();
