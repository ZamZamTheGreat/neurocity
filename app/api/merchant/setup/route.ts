...
export async function PATCH(request: Request) {
  ...
  const hours = Array.isArray(payload.hours) ? payload.hours.filter((item) => Number.isInteger(item.dayOfWeek) && item.dayOfWeek >= 0 && item.dayOfWeek <= 6).map((item) => ({ dayOfWeek: item.dayOfWeek, opensAt: item.closed ? null : item.opensAt || null, closesAt: item.closed ? null : item.closesAt || null, closed: item.closed === true })) : [];
  if (payload.isPublic && (!payload.tagline?.trim() || !payload.description?.trim() || !logoUrl || !bannerUrl || hours.length !== 7 || !payload.returnsPolicy?.trim())) return Response.json({ error: "Complete branding, description, all opening hours and the returns policy before publishing." }, { status: 409 });
  const values = { name, category, primaryCategory: category, contactName: payload.contactName?.trim() || null, contactEmail: email, contactPhone: payload.contactPhone?.trim() || null, website: payload.website?.trim() || null, pickupLocation: branchAddress, deliveryMode: payload.deliveryMode === "platform_managed" ? "platform_managed" : "merchant_managed", tagline: payload.tagline?.trim() || null, description: payload.description?.trim() || null, logoUrl: logoUrl || null, bannerUrl: bannerUrl || null, policies: { returns: payload.returnsPolicy?.trim() || "", shipping: payload.shippingPolicy?.trim() || "", privacy: payload.privacyPolicy?.trim() || "" }, contactOptions: { phone: payload.contactPhone?.trim() || "", email, website: payload.website?.trim() || "" }, fulfillmentMethods: [payload.pickupEnabled ? "pickup" : null, payload.deliveryEnabled ? "merchant_delivery" : null].filter(Boolean), isPublic: payload.isPublic === true, setupStep: 3 };
  const db = getDb();
  const result = await db.transaction(async (tx) => {
    ...
    await tx.insert(auditEvents).values({ actorRef: access.user.userId, action: "merchant.setup_updated", resourceType: "merchant", resourceId: String(access.merchantId), metadata: { isPublic: values.isPublic, category, setupStep: values.setupStep }, createdAt: new Date() });
