...
import { auditEvents, merchants, productVariants, products, storeBranches, variantInventory } from "../../../../../db/schema";
import { isMerchantCategory } from "../../../../../lib/merchant-categories";
import { requirePilotMerchant } from "../../auth";
import { commerceTypeForCategory, isCommerceType } from "../../../../../lib/commerce-templates";

type ImportRow = { name?: unknown; sku?: unknown; category?: unknown; commerceType?: unknown; description?: unknown; price?: unknown; salePrice?: unknown; brand?: unknown; collection?: unknown; variantSku?: unknown; variantTitle?: unknown; size?: unknown; sizes?: unknown; color?: unknown; variantPrice?: unknown; variantSalePrice?: unknown; stock?: unknown; stockBySize?: unknown };
...
export async function POST(request: Request) {
  ...
  const parsed = payload.rows.flatMap((row, index) => {
    ...
    const commerceType = clean(row.commerceType) || commerceTypeForCategory(category);
  ...
  const imported = await db.transaction(async (tx) => {
    ...
    await tx.update(merchants).set({ enabledCategories: [...new Set([...(merchant?.enabledCategories as string[] ?? []), ...productRows.map((row) => row.category)])], enabledCommerceTypes: [...new Set([...(merchant?.enabledCommerceTypes as string[] ?? []), ...productRows.map((row) => row.commerceType)])] }).where(eq(merchants.id, access.merchantId));
    for (const row of productRows) {
      const [product] = await tx.insert(products).values({ merchantId: access.merchantId, itemType: "product", commerceType: row.commerceType, commerceAttributes: {}, name: row.name, sku: row.sku, category: row.category, description: row.description, brand: row.brand, collection: row.collection, price: row.price, salePrice: row.salePrice, pricingModel: "fixed", status: "draft", availability: "available" }).returning();
