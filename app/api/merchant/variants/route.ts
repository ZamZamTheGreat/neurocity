...
export async function GET() {
  ...
  return Response.json({ variants: rows.map((row) => ({ ...row, storageImageUrl: row.imageUrl, imageUrl: row.imageUrl?.startsWith("r2://") ? `/api/merchant/variants/media?variantId=${row.id}` : row.imageUrl, stock: stock.filter((item) => item.variantId === row.id) })) });
}
...
export async function PATCH(request: Request) {
  const access = await requirePilotMerchant(["owner", "manager"]); if (!access) return Response.json({ error: "Owner or manager access required." }, { status: 403 });
  const payload = await request.json() as { id?: number; title?: string; size?: string; color?: string; price?: number; salePrice?: number | null; status?: string; onHand?: number; safetyStock?: number; imageUrl?: string | null };
  ...
  if (imageUrl?.startsWith("r2://") && !imageUrl.slice(5).startsWith(`merchants/${access.merchantId}/products/${current.productId}/variants/${current.id}/`)) return Response.json({ error: "Invalid variant image." }, { status: 403 });
  const [variant] = await db.update(productVariants).set({ title: payload.title.trim(), size: payload.size?.trim() || null, color: payload.color?.trim() || null, price: payload.price!, salePrice: payload.salePrice ?? null, status: payload.status!, imageUrl }).where(eq(productVariants.id, current.id)).returning();
