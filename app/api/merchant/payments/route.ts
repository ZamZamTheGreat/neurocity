...
export async function GET() {
  ...
  const now = Date.now();
  const amount = (statuses: string[], predicate?: (item: typeof settlements[number]) => boolean) => sumMoney(settlements.filter((item) => statuses.includes(item.status) && (!predicate || predicate(item))).map((item) => Number(item.netAmount)));
  const payableStatuses = ["unpaid", "scheduled", "due", "processing", "settled"];
  return Response.json({ settings: settingsFor(merchant?.paymentSettings), settlements: settlements.map((item) => ({ ...item, effectiveStatus: item.status === "scheduled" && item.dueAt && new Date(item.dueAt).getTime() <= now ? "due" : item.status })), summary: { pendingCustomerPayment: amount(["pending_payment"]), scheduled: amount(["scheduled"], (item) => Boolean(item.dueAt && new Date(item.dueAt).getTime() > now)), dueNow: amount(["unpaid", "due", "scheduled"], (item) => ["unpaid", "due"].includes(item.status) || Boolean(item.dueAt && new Date(item.dueAt).getTime() <= now)), processing: amount(["processing"]), awaitingSettlement: amount(["unpaid", "scheduled", "due", "processing"]), settled: amount(["settled"]), refundAdjustment: amount(["refund_required"]), grossSales: sumMoney(settlements.filter((item) => payableStatuses.includes(item.status)).map((item) => Number(item.grossAmount))) } }, { headers: { "cache-control": "no-store" } });
}
