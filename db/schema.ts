...
export const merchants = pgTable("merchants", {
  ...
  enabledCommerceTypes: jsonb("enabled_commerce_types").notNull().default([]),
...
export const advertisingCampaigns = pgTable("advertising_campaigns", {
  ...
  createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(),
  updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow(),
}, (table) => [index("idx_advertising_campaigns_placement_status").on(table.placement, table.status), index("idx_advertising_campaigns_merchant").on(table.merchantId), index("idx_advertising_campaigns_product").on(table.productId)]);
...
export const orders = pgTable("orders", { id: serial("id").primaryKey(), checkoutGroupId: integer("checkout_group_id").references(() => checkoutGroups.id, { onDelete: "set null" }), merchantId: integer("merchant_id").notNull().references(() => merchants.id), customerRef: text("customer_ref").notNull(), customerName: varchar("customer_name", { length: 160 }), customerEmail: varchar("customer_email", { length: 320 }), customerPhone: varchar("customer_phone", { length: 80 }), status: varchar("status", { length: 48 }).notNull().default("draft"), workflow: varchar("workflow", { length: 64 }).notNull().default("legacy_retail_v1"), workflowVersion: integer("workflow_version").notNull().default(1), orderVersion: integer("order_version").notNull().default(1), confirmationExpiresAt: timestamp("confirmation_expires_at", { withTimezone: true }), confirmedAt: timestamp("confirmed_at", { withTimezone: true }), paymentExpiresAt: timestamp("payment_expires_at", { withTimezone: true }), paymentStatus: varchar("payment_status", { length: 32 }).notNull().default("pending"), inventoryState: varchar("inventory_state", { length: 24 }).notNull().default("reserved"), paymentMethod: varchar("payment_method", { length: 60 }), paymentInstructions: jsonb("payment_instructions"), fulfillmentMethod: varchar("fulfillment_method", { length: 60 }), addressSnapshot: jsonb("address_snapshot"), customerNotes: text("customer_notes"), subtotal: doublePrecision("subtotal").notNull().default(0), deliveryFee: doublePrecision("delivery_fee").notNull().default(0), total: doublePrecision("total").notNull().default(0), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow() }, (table) => [index("idx_orders_checkout_group").on(table.checkoutGroupId), index("idx_orders_merchant_status").on(table.merchantId, table.status), index("idx_orders_confirmation_expiry").on(table.status, table.confirmationExpiresAt), index("idx_orders_payment_expiry").on(table.status, table.paymentExpiresAt), index("idx_orders_customer_ref").on(table.customerRef), index("idx_orders_customer_created").on(table.customerRef, table.createdAt), index("idx_orders_merchant_created").on(table.merchantId, table.createdAt), index("idx_orders_payment_status").on(table.paymentStatus), index("idx_orders_inventory_state").on(table.inventoryState)]);
...
export const orderItemInventoryAllocations = pgTable("order_item_inventory_allocations", { id: serial("id").primaryKey(), orderItemId: integer("order_item_id").notNull().references(() => orderItems.id, { onDelete: "cascade" }), inventoryId: integer("inventory_id").notNull().references(() => variantInventory.id), quantity: integer("quantity").notNull(), state: varchar("state", { length: 24 }).notNull().default("reserved"), updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow() }, (table) => [uniqueIndex("idx_order_item_inventory_allocation").on(table.orderItemId, table.inventoryId), index("idx_order_inventory_allocation_state").on(table.state)]);
...
export const orderWorkflowNotifications = pgTable("order_workflow_notifications", { id: serial("id").primaryKey(), notificationKey: varchar("notification_key", { length: 160 }).notNull(), checkoutGroupId: integer("checkout_group_id").references(() => checkoutGroups.id, { onDelete: "cascade" }), orderId: integer("order_id").references(() => orders.id, { onDelete: "cascade" }), audience: varchar("audience", { length: 32 }).notNull(), channel: varchar("channel", { length: 24 }).notNull().default("email"), status: varchar("status", { length: 24 }).notNull().default("processing"), attempts: integer("attempts").notNull().default(1), lastError: text("last_error"), sentAt: timestamp("sent_at", { withTimezone: true }), createdAt: timestamp("created_at", { withTimezone: true }).notNull().defaultNow(), updatedAt: timestamp("updated_at", { withTimezone: true }).notNull().defaultNow() }, (table) => [uniqueIndex("idx_order_workflow_notifications_key").on(table.notificationKey), index("idx_order_workflow_notifications_status").on(table.status, table.updatedAt), index("idx_order_workflow_notifications_order").on(table.orderId)]);
